...
module GtkDriver
  ...
  # How many extra ticks a screenshot may wait for GTK to draw it a frame.
  #
  # Offscreen, a widget only offers a render node once GTK has drawn it and
  # considers it clean, and there is no way to ask for that from outside the
  # frame clock. Retrying across ticks takes most runs to a full set of
  # screenshots; a minority of runs still lose some, which is a limitation of
  # snapshotting a surfaceless widget and not a fault in the app under test.
  # The checks are unaffected — they never depend on rendering.
  SHOT_RETRIES = 30
  ...
  class Session
    ...
    def shot(name, widget = nil)
      if @shots.nil?
        raise 'no shots directory configured'
      end

      (widget || default_window).then do |target|
        if target.nil?
          raise 'no widget to screenshot'
        end
    ...
    # A widget only offers a render node when GTK has drawn a frame for it since
    # the last snapshot, and returning to the main loop is the only thing that
    # produces one. So a nil node is not a failure yet: the shot is queued and
    # retried on later ticks. Retrying in a loop here cannot work — nothing
    # redraws in between.
    def take_shot(name, target, path)
      render(target, path)
      puts "    shot #{path}"
      path
    rescue RuntimeError
      @pending_shots << [name, target, path, SHOT_RETRIES]
      nil
    end

    # Retries one queued shot, re-queueing it while it has attempts left.
    #
    # The queue is drained before any further step runs, so a retried shot still
    # documents the state the step that asked for it left behind. Do not
    # invalidate the widget here: a node is only available once GTK considers
    # the widget clean, so `queue_draw` makes the retry less likely to work,
    # not more.
    def drain_pending_shots
      name, target, path, remaining = @pending_shots.shift

      begin
        render(target, path)
        puts "    shot #{path} (retried)"
      rescue RuntimeError => e
        if remaining.positive?
          @pending_shots << [name, target, path, remaining - 1]
        else
          puts "    shot #{name} failed: #{e.class}: #{e.message}"
        end
      end
    ...
    def run
      ...
      # GskRenderer asserts it is not still realised when it is disposed, and a
      # failed assertion aborts the process — so the shared renderer has to be
      # released deliberately rather than left to the garbage collector.
      release_renderer
    ...
    # The paintable is held in an ivar for the duration of the render: a
    # GtkWidgetPaintable that Ruby collects between construction and `snapshot`
    # takes its render node with it, which showed up as roughly two in three
    # screenshots failing with a nil node for no reason related to the widget.
    # The paintable is fresh per shot — a cached one keeps handing back the
    # first frame it ever saw — but is held in an ivar for the duration of the
    # call: a GtkWidgetPaintable that Ruby collects between construction and
    # `snapshot` takes its render node with it, which showed up as roughly two
    # in three screenshots failing with a nil node.
    #
    # A widget only has a render node once it has been drawn, so shoot in a
    # later step than the one that changed it.

    # One renderer for the whole run, realised once. Realising and unrealising
    # a GskCairoRenderer per shot poisons the widget tree: the first couple of
    # screenshots come out, and every one after that finds a nil render node.
      def renderer
        @renderer ||= Gsk::CairoRenderer.new.tap { |gsk| gsk.realize(nil) }
      end
      ...
      def render(widget, path)
        ...
        begin
          Gtk::Snapshot.new.then do |snapshot|
            @paintable.snapshot(snapshot, widget.width, widget.height)
            snapshot.to_node.then do |node|
              if node.nil?
                raise 'widget produced no render node (not realised yet?)'
              end
      ...
      def report
        puts
        if @checks.positive?
          puts "#{@checks - @failed_checks.length} of #{@checks} checks passed"
        end
